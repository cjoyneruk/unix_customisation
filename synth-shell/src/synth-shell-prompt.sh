...
source "${HOME}/.config/synth-shell/bash-tools/color.sh"
source "${HOME}/.config/synth-shell/bash-tools/shorten_path.sh"
source "${HOME}/.config/synth-shell/config/synth-shell-prompt.custom.config"
		...
		if [[ -n "$branch" ]]; then
			...
			if [ "$SSP_GIT_UPDATE_PERIOD_MINUTES" -ge 0 ]; then
				...
				if [ -d "$dot_git" -a -e "${dot_git}/FETCH_HEAD" ]; then
					local git_last_update=$(stat -f "%Y" "${dot_git}/FETCH_HEAD")
				fi
